    ...
    for init in range(len(GEN_WELL_DATA)): # Iterate thorugh each experiment
        ...
        for k in range(cell_count):
            ...
            if (len(GFP_POOL) > 0):
                ...
                num_interactions = np.random.poisson(len(GFP_POOL) / PARAM_DICT['vMax']) # Calculate the number of virions that will visit the cell
                ...
                for a in range(len(VIRION_ATTACKERS)):
                    is_successful = Innoculation(VIRION_ATTACKERS[a], CELL_POOL[k], gamma=PARAM_DICT['gamma'], 
                                                 acc_dam=True, beta=PARAM_DICT['beta'])
                    total += 1
                    if is_successful:
